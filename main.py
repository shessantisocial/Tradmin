from tkinter import Listbox, Scrollbar, END, VERTICAL, ttk
...
def main_page():
    ...
    mainscreen = ctk.CTk()
    mainscreen.title('TRADMIN')
    mainscreen.geometry('1366x768')
    mainscreen.resizable(0, 0)

    # set_appearance_mode('dark')

    upperframe = CTkFrame(master=mainscreen, fg_color='#FFFFFF', width=650, height=85, corner_radius=0)
    ...
    sidebar = CTkFrame(master=mainscreen, fg_color='#FFFFFF', width=190, height=650, corner_radius=0)
    ...
    def show_status():
        ...
        history_tree.insert("", "end", values=(selected_vehicle.get(), status_text, selected_date))        
    ...
    bottomframe = CTkFrame(master=mainscreen, fg_color='#FFFFFF', width=650, height=200, corner_radius=0)
    ...
    mapframe = CTkFrame(master=mainscreen, fg_color='#FFFFFF', width=550, height=510, corner_radius=0)
    ...
    historyframe = CTkFrame(master=mainscreen, fg_color='#FFFFFF', width=650, height=510, corner_radius=0)
    historyframe.pack_propagate(0)
    historyframe.pack(fill='x', anchor='w', side='left')
    historylabel = CTkLabel(master=historyframe, text="HISTORY", font=('Arial Bold', 21), text_color='#0B2F4F')
    historylabel.pack(anchor='nw', pady=(29, 0), padx=27)
    ...
    history_tree = ttk.Treeview(master=historyframe, columns=("Vehicle", "Status", "Date"), show="headings", height=10)

    history_tree.heading("Vehicle", text="Vehicle")
    history_tree.heading("Status", text="Status")
    history_tree.heading("Date", text="Date")
    history_tree.column("Vehicle", width=150)
    history_tree.column("Status", width=100)
    history_tree.column("Date", width=100)

    history_tree.pack(anchor="nw", padx=27, pady=(10, 0), fill="x")
